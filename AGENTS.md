...
This is a local web app that monitors group chat messages via pluggable message sources (default: WeFlow HTTP API), classifies them with AI, deduplicates across groups, optionally OCRs attached images, and displays structured campus information (events, club recruitment, academics, second-hand trades, internships). Multiple sources can run concurrently (each source is a plugin, enabled via `PLUGINS` config).

### Data flow

```
WeFlow :5031
  ├─ SSE /api/v1/push/messages  →  plugins/weflow/client.py (stream_events) → sse.py (real-time)
  └─ REST /api/v1/messages       →  plugins/weflow/client.py → poller.py (backfill, BACKFILL_HOURS)
              ↓
        plugins/weflow/normalize.py  (SSE/REST → InternalMessage; pre-filter noise; image URLs)
              ↓
        pipeline.py   (skeleton: stage chain enrich → classify → dedup → insert → merge)
              ↓
        plugins/classify/engine.py   (batched → AI via ai_ports → plugins/ai_provider/engine.py)
              ↓
        plugins/dedup/engine.py      (embedding cosine pre-filter (optional) → AI semantic dedup)
              ↓
        db.py         (SQLite via aiosqlite)
              ↓
        realtime.py   (in-process pub/sub → /api/stream SSE → frontend)
              ↓
        server/       (FastAPI on :3000 — 子包：路由/中间件/媒体代理/静态托管)
              ↓
        ui/           (vanilla HTML/CSS/JS SPA, no framework)
```
...
| `briefdesk/realtime.py` | 进程内发布/订阅：`publish_items_updated()` 把入库事件推给所有订阅队列，由 server 的 `/api/stream` SSE 转发给前端。 |
| `briefdesk/status.py` | 应用运行时状态 + 消息源注册表：`set_status`/`get_status_info`/`is_syncing`、`register_source_client`/`get_source_client`、`set_listener`/`get_listener`。pipeline/poll_cycle 与 server 都只依赖本模块（不互相依赖），避免业务层反向依赖 HTTP 层。**相对时间展示已整体移到前端（P5）**：卡片行 `relativeTime` 与状态面板 `relativeSync` 均由前端按 `msg_time`/`lastSync` 自行计算（`relativeTimeStr`/`itemRelativeTime`/`syncRelativeText`），本模块只下发原始时间数据。 |
...
| `briefdesk/stages.py` | 管道阶段注册表与装配期上下文（P3 起）：StagePlugin 经 `ctx.register_stage` 注册（main 把端口接到本模块），pipeline 骨架按槽位（enrich → classify → dedup → post_insert）读取，同槽按 priority 升序；`set_context` 注入装配期 PluginContext（阶段 run(batch, ctx) 经此获得 `ctx.dedup` 等服务端口）。模块级单例，测试用 `reset()` 隔离。 |
...
| `briefdesk/plugin/base.py` | 插件最小契约：`Plugin` Protocol（name/version/dependencies + setup/activate/teardown 生命周期）、`PluginContext`（核心注入给插件的服务端口：config、事件总线，P2 起含 `register_source`，P3 起含 `register_stage` 与 `dedup` 服务端口，P4 起含 `ai` 服务端口，P5 起含 `register_router`/`register_plugin_assets`——默认 noop 未注入时静默丢弃）、异常 `PluginError`（致命）/`PluginDisabledError`（自禁用，非致命）。entry point 组名 `PLUGIN_GROUP = "briefdesk.plugins"`；加载约定：模块暴露 `plugin` 实例。能力协议：SourcePlugin（P2）/ StagePlugin 与 DedupService（P3）/ AIProvider（P4）/ WebPlugin（P5，router + asset_dir）全部已落。**内置插件类显式继承对应能力协议**（mypy 强制实现完整性；第三方插件亦可鸭子实现，manager 只做结构校验）。 |
| `briefdesk/plugin/manager.py` | `PluginManager`：发现（entry point 组 `briefdesk.plugins` + `PLUGIN_PATH` 目录扫描）→ `PLUGINS`/`PLUGINS_DISABLED` 过滤 → 依赖拓扑排序（未知依赖/依赖环降级 disabled）→ `setup_all`（HTTP 启动前）→ `activate_all`（服务器就绪后）→ `teardown_all`（逆序幂等）。单插件失败隔离（disabled/failed + 日志），`PLUGINS_REQUIRED` 名单内的失败抛 `PluginError` 致命。`infos()`（`/api/plugins` 数据源）每项含 `has_frontend`：按 `asset_dir()` 是否非 None 判定（异常安全回退 False），前端加载器据此跳过无前端资源插件的注入。 |
...
- **插件前端随插件包分发（P5）**：设置弹窗新增「插件」分组（`/api/plugins` 渲染名称/版本/状态/原因）；核心前端只留通用加载器 `loadPluginFrontends`——读取 `/api/plugins` 取 loaded 名单 → 隐藏所有 `[data-plugin-entry="<name>"]` 声明入口 → **仅对声明了前端资源的插件**（`has_frontend`，`asset_dir()` 非 None）注入 `/plugin-assets/<name>/ui.css`（样式）与 `/plugin-assets/<name>/ui.js`（脚本；无前端资源的插件不请求，避免 404 触发浏览器严格 MIME 检查告警）→ 调用 `window.briefdeskPlugins.<name>.init({isLoaded})`（异常仅 console.warn）。**插件的完整前端（DOM/样式/交互/入口）随插件包分发**：calendar 的按钮、视图容器、两个浮层全部由 `ui/ui.js` 自建，核心 `ui/` 不写死任何插件功能入口（`CoreFrontendBoundaryTest` 守卫）；核心提供两类插件扩展钩子——**视图钩子** `registerPluginView`（hash 路由 / fetchData 委派 / Esc 消费 / 侧边栏数据就绪通知，calendar 视图 `#calendar` 据此接入）与**行内扩展** `registerItemRowExtension`（renderItemRow/renderCard 动作区按钮与行末菜单渲染、handleRowAction 委派、文档点击关闭菜单、verifyItem 成功后 onVerify 通知——reminders 的提醒按钮/菜单/自动提醒据此接入），另设 `data-plugin-slot` 设置面板注入点（reminders 注入「自动提醒」控件）
...
|---|---|---|
| `PLUGINS` | `["*"]` | **JSON array** of enabled plugin names; `"*"` = all discovered. 消息源启用的唯一开关（weflow/qqflow 等源插件由本开关控制；不再使用 SOURCES） |
...
| `DEDUP_EMBED_FALLBACK_THRESHOLD` | `0.65` | 低置信复核阈值：余弦候选落在 [fallback, `DEDUP_EMBED_THRESHOLD`) 区间且无 normal 候选时，全员判 SAME 才判重（修复中段相似度真实重复被门禁静默漏判，如玉言事故 cosine 0.75 < 0.80） |
...
- 插件框架（P1 起）：依赖方向单向——核心与 `briefdesk/plugin/*` 永不静态 import `briefdesk.plugins.*`，由 `tests/test_no_core_imports_plugins.py` AST 守卫强制执行；插件经 entry point 组 `briefdesk.plugins` 或 `PLUGIN_PATH` 发现（约定模块暴露 `plugin` 实例）。装配顺序固定：setup（HTTP 启动前，DB 就绪后）→ activate（服务器就绪后）→ teardown（逆序幂等），消息源预热（去重缓存等）与监听启动等顺序约束由该阶段划分承载。P1 框架落地（main 未接入）；P2 起 main 经 PluginManager 装配：weflow/qqflow 为内置插件（`briefdesk/plugins/*`，pyproject entry points 注册），`SOURCES` 已移除，消息源启用完全走 `PLUGINS`/`PLUGINS_DISABLED`（源插件在 setup 阶段经 `ctx.register_source` 注册 `SourceRuntime`；零源启动报错）；**P3 管道阶段化**：OCR/分类/去重/合并为 StagePlugin（槽位 enrich/classify/dedup/post_insert，注册表 `briefdesk/stages.py`，骨架 `briefdesk/pipeline.py` 只做编排），跨插件契约（ClassifyResult/ClassifyOutcome/DedupResult/BatchContext）上移 `briefdesk/types.py`，核心删除卡片改经事件总线 `EVENT_ITEMS_DELETED` 通知（去重插件订阅清缓存），`ctx.dedup` 服务端口供 merge 阶段同步缓存；**P4 AI 供应商插件**：`ai/client.py`+`ai/embed.py` 迁为 `plugins/ai_provider/`（`Provider` 显式实现 AIProvider），引擎改经 core `briefdesk.ai_ports` 端口函数调用（未注册时 chat 抛错、嵌入启用性安全回退），classify/dedup/merge 声明依赖 ai_provider（被禁用时随依赖降级，pipeline 骨架对"阶段缺失"整批保留不标记防消息丢失）；**P5 Web 插件**：`WebPlugin` 协议（router + asset_dir），日历/提醒路由迁为 `plugins/{calendar,reminders}` 示范，`/api/plugins` 元数据 + `/plugin-assets/{name}/{path}` 静态资源由核心提供，`include_plugin_router` 展开 APIRoute 插到 SPA mount 之前（新版 Starlette 惰性 `_IncludedRouter` 会被 SPA 兜底截胡），前端设置区插件分组与入口可见性按元数据驱动；**插件完整前端随插件包分发**（`ui/ui.js` 约定：IIFE 暴露 `window.briefdeskPlugins.<name>.init(api)`，DOM/样式/交互全在插件包内——calendar 自建按钮/视图容器/浮层并注册 `registerPluginView` 视图钩子接入核心 hash 路由与刷新联动；reminders 注册 `registerItemRowExtension` 行内扩展接入卡片动作区与 `handleRowAction`、`data-plugin-slot` 注入设置面板控件、自建到期轮询定时器；`ui.css` 随 `ui.js` 一并注入；核心 `ui/` 零插件前端残留，由 `CoreFrontendBoundaryTest` 守卫）
