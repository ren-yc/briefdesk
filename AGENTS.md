...
|---|---|---|
| `PLUGINS` | `["*"]` | **JSON array** of enabled plugin names; `"*"` = all discovered. 消息源启用的唯一开关（weflow/qqflow 等源插件由本开关控制；不再使用 SOURCES）。声明 `default_disabled = True` 的插件（如实验性 benchmark）默认不随 `"*"` 加载，需显式列名（`PLUGINS=["*", "benchmark"]`）才启用 |
