...
class TestDefaultDisabled:
    ...
    async def test_not_loaded_by_default(self):
        ...
        assert "未启用" in rec.reason  # /api/plugins 可见原因
    ...
    async def test_loaded_when_explicit(self):
        ...
        manager.register(BenchmarkPlugin())
        await manager.setup_all(_bare_ctx())
        assert "benchmark" in manager.loaded
        assert manager.records()["benchmark"].status == "loaded"
...
class TestPluginSetup:
    async def test_benchmark_plugin_registers_router_and_assets(self):
        ...
        assert "/api/benchmark/cases" in paths
        assert "/api/benchmark/import-current" in paths
        assert "/api/benchmark/run" in paths
        assert "/api/benchmark/report" in paths
        assert "/api/benchmark/record" in paths
        assert "/api/benchmark/export-recorded" in paths
        assert assets == [("benchmark", str(BenchmarkPlugin().asset_dir()))]
        assert BenchmarkPlugin.dependencies == ("ai_provider",)
        # 同时注册为阶段插件（slot=post_insert，priority=1，在 merge 阶段之后）
        assert len(stages) == 1
        assert stages[0].slot == "post_insert"
        assert stages[0].priority == 1
...
class TestStore:
    ...
    async def test_export_list_delete(self):
        ...
        assert path == self.cases_dir / "classify.fromweb.json"
        assert path.exists()
        cases = await bench_store.list_fromweb("classify")
        assert len(cases) == 1
        assert cases[0]["id"] == "bench-abc"
        assert cases[0]["feature"] == "classify"
        assert await bench_store.list_fromweb("dedup") == []
        # 文件为 DatasetFile 顶层结构（与手写数据集同构）
        raw = json.loads(path.read_text(encoding="utf-8"))
        assert raw["feature"] == "classify"
        ...
        cases = await bench_store.list_fromweb("classify")
        assert [c["id"] for c in cases] == ["bench-xyz"]

        deleted = await bench_store.delete_all_fromweb("classify")
        assert deleted == 1
        assert not path.exists()
        assert await bench_store.list_fromweb() == []
    ...
    async def test_export_invalid_case_rejected(self):
        ...
        assert not (self.cases_dir / "classify.fromweb.json").exists()
    ...
    async def test_delete_all_by_feature(self):
        ...
        assert {c["feature"] for c in remaining} == {"dedup", "title"}
        assert await bench_store.delete_all_fromweb() == 2
    ...
    async def test_list_invalid_file_skipped(self):
        ...
        assert await bench_store.list_fromweb("classify") == []
    ...
    async def test_load_web_cases(self):
        ...
        assert await bench_engine.load_web_cases("dedup") == []
...
class TestRecorder:
    ...
    def test_record_batch_builds_cases(self):
        ...
        assert st["counts"]["dedup"] == 2
        assert st["counts"]["merge"] == 2
        assert st["counts"]["title"] == 1
        assert st["counts"]["classify"] == 0
        # 命中/未命中的期望与判定结论一致
        dedup = bench_recorder._cases["dedup"]
        assert sorted(c["expected"]["same"] for c in dedup) == [False, True]
        assert dedup[0]["query"]["msg_id"] == "m2"
        assert dedup[0]["items"][0]["msg_id"] == "i1"
        merge = bench_recorder._cases["merge"]
        assert sorted(c["expected"]["merge"] for c in merge) == [False, True]
        # title 用例：期望 = 合并后 key_info 关键词，old_title = 重拟前标题
        title = bench_recorder._cases["title"]
        assert title[0]["expected"]["keywords"] == ["价格", "运费"]
        assert title[0]["old_title"] == "团购"
        assert title[0]["message"]["content"] == "团购\n补充"
    ...
    def test_dedup_no_candidate_skipped(self):
        ...
        bench_recorder.record_batch(batch)
        assert bench_recorder.stats()["total"] == 0
    ...
    def test_cap_drops_new_records(self):
        with patch.object(bench_recorder, "_MAX_CASES_PER_FEATURE", 2):
            for _ in range(3):
                ...
                bench_recorder.record_batch(batch)
        assert bench_recorder.stats()["counts"]["dedup"] == 2
    ...
    async def test_stage_run_records_only_when_enabled(self):
        ...
        await plugin.run(b, ctx)
        assert bench_recorder.stats()["total"] == 1
    ...
    async def test_export_recorded_writes_and_clears(self):
        ...
        assert result["counts"]["dedup"] == 1
        assert result["total"] == 1
        assert set(result["paths"]) == {"dedup"}
        assert Path(result["paths"]["dedup"]).exists()
        # 导出后清空累积器
        assert bench_recorder.stats()["total"] == 0
        cases = await bench_store.list_fromweb("dedup")
        assert len(cases) == 1
        assert cases[0]["expected"]["same"]
    ...
    async def test_export_empty_keeps_existing_files(self):
        ...
        cases = await bench_store.list_fromweb("dedup")
        assert [c["id"] for c in cases] == ["keep"]
...
class BuildCasesTest(unittest.TestCase):
    ...
    def test_basic_batch(self):
        ...
        assert case["expected"][0]["index"] == 0
        assert case["expected"][0]["category"] == "活动通知"
        assert case["expected"][0]["start"] == "2026-04-20 19:00"
        assert case["expected"][1]["index"] == 1
        assert case["expected"][1]["category"] == "交易"
        assert case["messages"][0]["content"] == "活动内容一"
        assert "由设置导出" in case["note"]

    def test_batch_split_over_cap(self):
        rows = [self._row(f"i{i}", "活动通知", f"内容{i}") for i in range(105)]
        cases = bench_store.build_classify_cases_from_rows(rows)
        assert len(cases) == 2
        assert len(cases[0]["messages"]) == 100
        assert len(cases[1]["messages"]) == 5
        # 第二用例的期望 index 重新从 0 计
        assert cases[1]["expected"][0]["index"] == 0

    def test_empty_rows(self):
        assert bench_store.build_classify_cases_from_rows([]) == []
    ...
    def test_classify_ignored_as_noise(self):
        ...
        assert [m["msg_id"] for m in case["messages"]] == ["msg-i1", "msg-i2", "msg-i3"]
        assert len(case["expected"]) == 1
        assert case["expected"][0]["index"] == 0
        assert case["expected"][0]["category"] == "活动通知"
        assert "含 2 条噪声（已忽略）" in case["note"]
    ...
    def test_ignored_excluded_from_title_dedup_merge(self):
        ...
        assert len(titles) == 2
        assert "msg-i2" not in [t["message"]["msg_id"] for t in titles]
        ...
        assert len(merges) == 1
        assert merges[0]["head"]["msg_id"] == "msg-i1"
        assert merges[0]["tail"]["msg_id"] == "msg-i3"
    ...
    def test_title_cases(self):
        ...
        assert kw_case["expected"]["keywords"] == ["十佳歌手", "报名"]
        assert kw_case["old_title"] == "标题-i1"
        assert kw_case["key_info"] == "十佳歌手, 报名"
        title_case = next(c for c in cases if "title" in c["expected"])
        assert title_case["expected"]["title"] == "标题-i2"
    ...
    def test_dedup_cases(self):
        ...
        assert len(trues) == 1
        assert trues[0]["items"][0]["msg_id"] == "msg-a"
        assert trues[0]["query"]["msg_id"] == "msg-b"
        # 时间相邻且类别不同（b-活动 与 c-交易）→ same=false；同类别对跳过
        assert len(falses) == 1
        assert falses[0]["items"][0]["msg_id"] == "msg-b"
        assert falses[0]["query"]["msg_id"] == "msg-c"
    ...
    def test_merge_cases(self):
        ...
        assert len(trues) == 1
        assert trues[0]["head"]["msg_id"] == "msg-h1"
        assert trues[0]["tail"]["msg_id"] == "msg-t1"
        assert len(falses) == 1
        assert falses[0]["head"]["msg_id"] == "msg-h2"
        assert falses[0]["tail"]["msg_id"] == "msg-t2"
...
class TestImportCurrent:
    ...
    async def test_import_current_exports_four_features(self):
        ...
        assert result["counts"] == {"classify": 1, "dedup": 4, "merge": 2, "title": 7}
        assert result["total"] == 14
        assert result["cards"] == 8  # 有效卡片（忽略行不计入）
        assert result["messages"] == 8  # classify 消息数（7 正样本 + 1 噪声）
        assert result["noise"] == 1  # 已忽略卡片 → classify 噪声样本
        assert result["skipped_no_category"] == 1
        # 四个文件全部导出且计数一致
        assert set(result["paths"]) == {"classify", "dedup", "merge", "title"}
        for f in ("classify", "dedup", "merge", "title"):
            path = self.cases_dir / f"{f}.fromweb.json"
            assert Path(result["paths"][f]) == path
            raw = json.loads(path.read_text(encoding="utf-8"))
            assert raw["feature"] == f
            assert len(raw["cases"]) == result["counts"][f]
        ...
        assert title_only == [{"title": "十佳歌手报名截止"}]  # r6 无 key_info
        all_kw = [tuple(c["expected"]["keywords"]) for c in tt["cases"] if "keywords" in c["expected"]]
        assert len(all_kw) == 6
        assert ("自行车", "二手") in all_kw
        assert ("十佳歌手", "报名") in all_kw
        assert ("考研数学",) in all_kw
    ...
    async def test_import_current_no_cases_skips_export(self):
        ...
        assert result["counts"] == {"classify": 0, "dedup": 0, "merge": 0, "title": 0}
        assert result["total"] == 0
        assert result["paths"] == {}
        cases = await bench_store.list_fromweb("classify")
        assert [c["id"] for c in cases] == ["keep"]
