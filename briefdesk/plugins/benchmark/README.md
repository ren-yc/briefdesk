...
插件**默认禁用**（实验性工具，声明 `default_disabled`，`PLUGINS=["*"]` 不会
加载它）：需在 `.env` 中显式列名启用，如 `PLUGINS=["*", "benchmark"]`
（或仅 `PLUGINS=["benchmark"]`；`PLUGINS_DISABLED` 仍优先）。启用后随应用
加载，设置弹窗 → **「关于」面板 → 「基准测试」区块**：
