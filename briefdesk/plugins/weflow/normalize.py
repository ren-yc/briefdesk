...
from briefdesk.plugins._common.article_parser import (
    _article_messages,
    _is_appmsg_content,
    parse_appmsg_xml,
)
...
# ── 文章卡片 XML 解析（共享实现见 _common.article_parser）──
# parse_appmsg_xml / _is_appmsg_content / _article_messages 从
# briefdesk.plugins._common.article_parser 导入，与 weflow-legacy 共用单一实现；
# poller 经本模块间接导入 parse_appmsg_xml / _is_appmsg_content（re-export）。
