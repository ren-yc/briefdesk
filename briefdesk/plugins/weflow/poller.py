...
# 丢弃，不进 OCR 也不入库）。依据：weflow-server（Rust）messages.rs 的
# `export_jobs.truncate(200)` —— 安装版 WeFlow（:5031）与 qqflow-server 无此限制。
_PAGE_LIMIT = 200
...
async def poll(
    ...
    *,
    ...
    if config.ignore_self and total_raw and not saw_is_send_field:
        logger.warning(
            "IGNORE_SELF=true 但本轮消息均无 isSend 字段：当前 weflow-server "
