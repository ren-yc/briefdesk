...
async def _run() -> None:
    ...
    # 1.5 清理过期的已忽略条目（IGNORED_EXPIRY_HOURS = 0 表示禁用）
    ...
    # 7. 等待服务器退出，随后统一清理 —— 唯一清理点，
    # 正常退出（信号触发 should_exit）与异常路径都必达。
    # 清理顺序：同步任务取消 → 插件（消息源监听 + 客户端）→ 数据库 → 残留任务。
    try:
        await server_task
    finally:
        shutdown_start = time_module.perf_counter()
        # 幂等：信号路径已 set；KeyboardInterrupt/异常路径靠这里兜底，
        # 否则 uvicorn 优雅退出会干等 /api/stream 流（最多 5s 超时强杀）。
