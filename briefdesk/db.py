...
# 默认分类类别（名称, 描述, 颜色），共 13 类。描述即各类别的默认 prompt；
...
DEFAULT_CATEGORIES: list[tuple[str, str, str]] = [
    ...
    ("社团招新", "组织公开吸纳长期成员身份。收录：①组织名称②吸收成员的表达（纳新、加入我们）③报名方式或联系方式任一。排除：单场活动的观众召集、专项志愿服务招募、企业岗位招聘、临时结伴找同伴的帖子。判别：通过后获得持续成员身份→本类；学期制轮值岗属获得身份，收录；以纳新为目的的新生体验课属本类。", "#7C3AED"),
    ("学术", "传播研究性内容本身。收录：①研究实质内容（科研进展、研究成果、学术观点、课题组动态，含课题组研究生招生）②正文或摘要可见即算内容自足，纯祝贺跟帖不算。排除：讲座会议比赛的时间地点公告、课程作业考试安排、非研究成果类文件的领取渠道、论文代写等有偿服务。判别：价值在知识本身→本类；学术讲座录像与成果相关资料归本类，通用课程工具娱乐类电子资料不属本类；转发看内容不看载体。", "#059669"),
    ("交易", "物品、票券或可交付虚拟物品（账号、激活码、网盘资料视同票券）的所有权或有偿使用权随对价转移，含出卖、转让、求购、交换、出租、求租。收录：①标的物②对价（价格、交换条件、任何明示金钱期待，含象征性金额与以物易物）③联系方式或价格私聊任一。排除：零对价赠送、失物寻回认领、房屋床位居住权流转、仅为跑腿劳务付酬的情形、依赖成团人数优惠的拼单接龙。判别：自身标价、以物易物，或零元但以支付金钱、物品、请客为取得前提→本类；抽奖与先到先得的纯赠送不属于本类；跑腿费与商品混合按服务主导归劳务承接；商家清仓属经营性推广，不收。", "#D97706"),
    ("实习", "走正式招聘流程建立雇佣关系的就业岗位信息，含实习、校园招聘、内推，不论有无薪酬。收录：①企业或机构名②正式投递方式（简历、网申、内推码）——此两项必须满足；岗位方向、时间、薪资、协议至少其一。排除：只有宣讲会招聘会日程而无岗位细节的公告、按次或按时计酬的零散短工与有偿散活、课题组研究生招生、学生组织纳新。判别：需投递简历、官网申请或签订雇佣文书（劳动合同、实习协议）→本类；无此类流程的一切其余受酬劳动不属于本类。", "#DB2777"),
    ("失物招领", "找回或归还特定原物，不含任何买卖。收录：①物品特征②拾获或丢失地点——此两项必须满足；拾获或丢失时间、认领方式至少其一。排除：标价出售求购交换、免费赠送他人、以出租使用为目的的物品信息。判别：酬金仅为寻回激励不影响归类；借用后需归还的请求不在本类。", "#DC2626"),
    ("求助互助", "不以报酬为前提、一人即可完成的具体帮助请求，含借还、代取代领、顺路捎带、答疑、填问卷点赞助力。收录：①具体需求②时间地点或交付方式③致谢或联系方式任一；提问式求助含具体场景与交付预期的同样收录。排除：支付报酬的雇请代办、花钱购买求购、需多人凑齐才成行的召集、无具体事项的单纯提问寒暄、以抽奖为主要吸引的问卷助力帖。判别：互惠承诺与答谢礼物不是报酬；需归还的借用归本类；邀人抽奖领券是对方受益，不属本类。", "#EA580C"),
    ("组队拼团", "凑同伴才成行的召集，含比赛组队、约球搭子、拼车出行、拼单团购。收录：①事项内容②还缺人数或成团条件③时间与联系方式任一。排除：加入组织成为成员的纳新、面向观众到场参与的单场事件通告、单人即可完成的帮忙、受酬劳动岗位、无偿服务者召集。判别：核心诉求是人没齐；AA分摊成本不是付酬，拼车找同行归本类；出钱请人专程接送归劳务承接。", "#0891B2"),
    ("兼职家教", "按次或按时计酬（含时薪、日薪、排班制）的短期劳务与服务承接，含家教、发单、设计剪辑、有偿代取、付费被试、付费捎带；发布找人干活的与个人承接的都收录。收录：①劳务内容②报酬或计酬方式③时间或联系方式任一。排除：需投递简历走正式流程的就业岗位、无偿志愿服务、不涉报酬的互助帮忙、商品自身的出售转让。判别：仅为跑腿服务付酬→本类；报酬是承接前提是关键；实验被试出让时间与数据属本类；简易劳务约定不属于雇佣文书。", "#CA8A04"),
    ("免费福利", "零对价的赠送、领取、分享，含赠品、优惠券、会员码、名额、通用课程工具娱乐类电子资料。收录：①给什么②如何领取③无需付款且领取独立于活动。排除：任何付款或以旧换新的获取方式、以参加活动为前提的现场权益、失物归还认领、研究成果类专业资料、以支付金钱物品请客补偿为取得前提的赠送。判别：给不特定的任何人→本类；抽奖话费消费券按本类；商业引流的赠品不收。", "#16A34A"),
    ("房屋租售", "一段时期居住权的流转，含租房、转租、合租找室友、床位互换，不论有偿或互换。收录：①房源床位或需求②位置③租金分摊或联系方式任一。排除：家具家电等动产的买卖转让、求职招聘信息、无偿借用住宿情形。判别：取得一段时期居住权即入本类，有无租金在所不论。", "#9333EA"),
    ("志愿公益", "为特定活动或项目提供无偿服务的召集，含志愿者、工作人员、礼仪、引导员，允许持续多日的专项。收录：①服务角色②服务场合③时间与报名方式任一。排除：支付工资报酬的用工、加入组织成为成员、面向观众或参赛者的单场赛事通告。判别：按次或按时计酬（工资日薪时薪件酬）才归劳务承接，餐食交通补贴不论定额或实报实销均不影响无偿认定，志愿时长证书纪念品亦然；活动方现场找帮手属本类。", "#0D9488"),
    ("奖助申报", "提交材料经评审获得资金的申报机会，含奖学金、助学金、补贴、资助项目。收录：①项目名称与资助方②面向对象与条件③申报方式或截止时间任一。排除：现场竞技展演类比赛公告、劳动换工资的岗位、无需申报直接领取的福利。判别：路径为交材料、评审、发放；竞赛含奖金仍按事件处理；两种性质兼有默认按事件处理。", "#BE185D"),
]
...
async def _seed_default_categories(db: aiosqlite.Connection) -> None:
    """类别表为空时播种默认分类（当前 13 类；用户删光重启后恢复，需求已确认）。
